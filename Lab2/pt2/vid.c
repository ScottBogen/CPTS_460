...
int show_bmp(char *p, int startRow, int startCol)
{
   ...
   img_width = *q;                 // width in pixels
   img_height = *(q + 1);           // img_height in pixels

   p += 54;                // p point at pixels now
   // but the picture is up-side DOWN
   rsize = 4*((3*img_width+3)/4);     // row size is a multiple of 4 bytes
   p += (img_height-1)*rsize;         // last row of pixels

   for (row=startRow; row<startRow + img_height; row++) {
     pp = p;
     i = 0;
     for (column=startCol; column<startCol + img_width; column++) {
         b = *pp;
	       g = *(pp+1);
	       r = *(pp+2);
         pixel = (b<<16) + (g<<8) + r;
